...
function safeSource(url: string) {
  try { const parsed = new URL(url); return parsed.protocol === 'https:' && parsed.hostname === 'europepmc.org' ? url : undefined; }
...
function Paper({ passage, quote }: { passage: Passage; quote?: string }) {
  ...
  return <details className="paper">
    <summary>
      ...
      <span className={`access ${passage.access_type}`}>{passage.access_type === 'full_text' ? 'Full text' : 'Abstract only'}</span>
    ...
    <div className="paper-body"><span className="eyebrow">{passage.section} · exact source passage</span>
      ...
      <a href={safeSource(passage.source_url)} target="_blank" rel="noreferrer">Read the original paper ↗</a>
...
export default function Evidence({ item, index, mock }: { item: ClaimResult; index: number; mock: boolean }) {
  ...
  return <article className="claim-card">
    ...
    {item.provenance ? <div className="retrieval-line">
      <span>{item.provenance.papers_found} papers discovered</span><span>·</span>
      <span className={item.provenance.retrieval_mode === 'keyword_only' ? 'degraded' : ''}>
        {item.provenance.retrieval_mode === 'hybrid' ? 'Elastic hybrid retrieval' : 'Keyword only · degraded retrieval'}</span>
    </div> : null}
    ...
    {item.status === 'incomplete' && item.discovered_sources?.length ? <details className="limitations"><summary>Sources discovered before the interruption</summary>
      <p>These papers were discovered but not successfully ranked or assessed. No verdict is based on them.</p>
    ...
    {item.provenance ? <details className="provenance"><summary>Research trail</summary><p>{item.provenance.provider} · {item.provenance.searched_at}</p>
      <code>{item.provenance.query}</code><p>{item.provenance.cache_hits} papers reused from the source cache.</p></details> : null}
  </article>;
}
